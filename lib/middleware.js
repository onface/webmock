...
module.exports = function (req, res, next, own) {
    ...
    setTimeout(function () {
        ...
        if (typeof targetSettins.baseData === 'object' && targetSettins.baseData[rType]) {
            debug('extend baseData')
            response.data = extend(true, response.data, targetSettins.baseData[rType])
        }
